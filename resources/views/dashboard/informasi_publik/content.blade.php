@section('informasi_publik')
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="forms-sample" action="{{ route('informasi.simpan') }}" method="post" enctype="multipart/form-data">
                        @csrf
    ...
    <div class="content-wrapper">
        ...
        <div class="container-fluid d-flex justify-content-end">
            <form action="{{ route('informasi.cariDashboard') }}" method="get" class="d-flex">
                @csrf
                            ...
                            <div class="modal fade" id="edit_{{ $x->id}}" tabindex="-1" role="dialog" aria-labelledby="tambah" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form class="forms-sample" action="{{ route('informasi.update', $x->id) }}" method="POST" enctype="multipart/form-data"> @csrf @method('PUT')
                                                <div class="form-group row">
                                                    <label for="nama_file" class="col-sm-3 col-form-label">Nama File</label>
                                                    <div class="col-sm-9">
                                                        <input type="text" class="form-control" name="nama_file" value="{{ $x->nama_file}}" id="nama_file_ubah" placeholder="Nama File">
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                                    <label for="tipe" class="col-sm-3 col-form-label">Tipe</label>
                                                ...
                                                <div class="form-group row">
                                                    <label for="Tanggal" class="col-sm-3 col-form-label">Tanggal Upload</label>
                                                    <div class="col-sm-9">
                                                        <input type="date" name="tgl_file" value="{{ $x->tgl_file }}" class="form-control" id="tanggal_ubah" placeholder="Tanggal">
                                                    </div>
                                                </div>
                                                <div class="form-group row">
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <form action="{{ route('informasi.delete', $x->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
                ...
                <ul class="pagination pagination-secondary justify-content-end">
                    ...
                    @for($i = 1; $i <= $lastPage; $i++)
                        <li class="page-item {{ $informasi->currentPage() == $i ? 'active' : '' }}">
                            <a class="page-link" href="{{ $informasi->appends(['judul' => request('judul')])->url($i) }}">{{ $i }}</a>
                        </li>
                    @endfor
        
                    @if($informasi->nextPageUrl())
                        <li class="page-item">
                            <a class="page-link" href="{{ $informasi->appends(['judul' => request('judul')])->nextPageUrl() }}" aria-label="Next">
